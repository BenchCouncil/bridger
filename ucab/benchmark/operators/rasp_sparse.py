...
def run_graph(data, model, batch_size, flag_clf):
    """
    ...
    for i in range(n_batch):
        ...
        model.run()
        c = time.perf_counter()
        if(flag_clf in ["forest_reg"]):
            out[start:end] = model.get_output(0).asnumpy()[:,0]
        else:
...
def build_rewriting(sklearn_func, dataset, model_dir, data_dir, out_dir, batch_size=None):
    ...
    for target in target_list:
        ...
        for sparse_type in  ["csr", "bsr", "csc", "coo", "dia", "lil"]:
            ...
            filename = os.path.join(out_dir, func_name + "_" + sparse_type + "_" + str(batch_size))
            model.save_model(filename)
    return 0
...
def run_rewriting(df, sklearn_func, dataset, model_dir, data_dir, batch_size):
    ...
    for target in target_list:
        if(func_name in ["RandomForestRegressor", "ExtraTreesRegressor"]):
            flag_clf = "forest_reg"
        else:
            flag_clf = "clf"
        print("base")
        ...
        out, IO_time, computation_time = run_graph(data, model, batch_size, flag_clf)
        ucab_time = IO_time + computation_time
        results.append(ucab_time)
        for sparse_type in  ["csr", "bsr", "csc", "coo", "dia", "lil"]:
            print(sparse_type)
            filename = os.path.join(model_dir, func_name + "_" + sparse_type + "_" + str(batch_size))
            model = load_lib(filename)
...
#build_rasp(models, datasets, target_list, num_repeat, model_dir, "test_datasets", "rasp_model", 1721)
