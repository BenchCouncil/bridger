...
def _rewriting(df, sklearn_func, dataset, model_dir, data_dir, batch_size=None):
    ...
    for target in target_list:
        ...
        load_time, exec_time, store_time, take_time, ucab_time = bench_ucab(model, 1, data, check_flag=False)
        results.append(ucab_time)
        model = build_model(clf, data.shape, sparse_replacing="bsr", dtype_converting=True, batch_size=batch_size, target=target)
        load_time, exec_time, store_time, take_time, ucab_time = bench_ucab(model, 1, data, check_flag=False)
        results.append(ucab_time)
        model = build_model(clf, data.shape, sparse_replacing="dia", dtype_converting=True, batch_size=batch_size, target=target)
        load_time, exec_time, store_time, take_time, ucab_time = bench_ucab(model, 1, data, check_flag=False)
        results.append(ucab_time)
