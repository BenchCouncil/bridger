...
from ucab.algorithms.preprocessing import binarizer,normalizer,label_encoder
from ucab.algorithms.scaler import min_max_scaler,max_abs_scaler,standard_scaler,robust_scaler
from ucab.algorithms.linear import linear_binary_classification
from ucab.algorithms.svm import linear_kernel_svr,sigmoid_kernel_svr,poly_kernel_svr,rbf_kernel_svr,linear_kernel_svc,sigmoid_kernel_svc,poly_kernel_svc,rbf_kernel_svc
from ucab.utils.supported_ops import preprocessing_op,linear_clf,linear_reg,tree_clf,tree_reg,ensemble_clf,ensemble_reg,svm_clf,svm_reg
from ucab.utils.name_parser import func_name_parser
from ucab.utils.tree_common import convert_decision_tree,convert_random_forest,convert_gbdt_feature
import tarfile
from tvm.topi.sparse.utils import convert_model_dense_to_sparse
from tvm.contrib.debugger import debug_executor
import time
from ucab.utils import tree_common
from ucab.algorithms.tree import decision_tree_classifier,decision_tree_regressor
from ucab.algorithms.forest import random_forest_classifier,random_forest_regressor,forest_feature_gemm_dense
...
class BaseModel:
    ...
    def run(self, data, breakdown=False):
        if((self.flag_clf in ["tree_clf", "tree_reg", "forest_clf", "forest_reg"]) and self.target != "cuda"):
        #if(self.flag_clf in ["tree_clf", "tree_reg", "forest_clf", "forest_reg"]):
            data = data.asnumpy()
            print(data.shape)
            ...
            for i in range(n_batch):
                ...
                self.model.run()
                c = time.perf_counter()
                if(self.flag_clf in ["forest_reg"]):
                    out[start:end] = self.model.get_output(0).asnumpy()[:,0]
                else:
...
class TreeModel(BaseModel):
    ...
    def _parse_params(self):
        if(self.sparse_replacing in ["csr", "bsr", "csc"]):
        ...
        elif(self.sparse_replacing == "coo"):
            S_data, S_row, S_col, T, B, L = convert_decision_tree(self.batch_shape[1], self.sklearn_model, self.flag_clf, self.dtype, self.target, self.sparse_replacing, self.dtype_converting)
            self.L = L.asnumpy()
            self.internal_node = T.shape[0]
            self.leaf_node = B.shape[0] 
            return L, S_data, S_row, S_col, T, B
        elif(self.sparse_replacing == "dia"):
            S_data, S_offsets, T, B, L = convert_decision_tree(self.batch_shape[1], self.sklearn_model, self.flag_clf, self.dtype, self.target, self.sparse_replacing, self.dtype_converting)
            self.L = L.asnumpy()
            self.internal_node = T.shape[0]
            self.leaf_node = B.shape[0] 
            return L, S_data, S_offsets, T, B
        elif(self.sparse_replacing == "lil"):
            S_data, S_rows, S_non_zero_cols, T, B, L = convert_decision_tree(self.batch_shape[1], self.sklearn_model, self.flag_clf, self.dtype, self.target, self.sparse_replacing, self.dtype_converting)
            self.L = L.asnumpy()
            self.internal_node = T.shape[0]
            self.leaf_node = B.shape[0] 
            return L, S_data, S_rows, S_non_zero_cols, T, B
        else:
    ...
    def _parse_params(self):
        if(self.flag_clf == "forest_clf"):
            if(self.sparse_replacing in ["csr", "bsr", "csc"]):
                ...
                S_data, S_row, S_col, T, B, step, L, classes = convert_random_forest(self.batch_shape[1], self.sklearn_model, self.flag_clf, self.dtype, self.target, self.dtype_converting, self.sparse_replacing)
            elif(self.sparse_replacing == "dia"):
                S_data, S_offsets, T, B, step, L, classes = convert_random_forest(self.batch_shape[1], self.sklearn_model, self.flag_clf, self.dtype, self.target, self.dtype_converting, self.sparse_replacing)
            elif(self.sparse_replacing == "lil"):
                S_data, S_rows, S_non_zero_cols, T, B, step, L, classes = convert_random_forest(self.batch_shape[1], self.sklearn_model, self.flag_clf, self.dtype, self.target, self.dtype_converting, self.sparse_replacing)
            else:
            ...
            if(self.sparse_replacing in ["csr", "bsr", "csc"]):
                ...
                S_data, S_row, S_col, T, B, step, L = convert_random_forest(self.batch_shape[1], self.sklearn_model, self.flag_clf, self.dtype, self.target, self.dtype_converting, self.sparse_replacing)
            elif(self.sparse_replacing == "dia"):
                S_data, S_offsets, T, B, step, L = convert_random_forest(self.batch_shape[1], self.sklearn_model, self.flag_clf, self.dtype, self.target, self.dtype_converting, self.sparse_replacing)
            elif(self.sparse_replacing == "lil"):
                S_data, S_rows, S_non_zero_cols, T, B, step, L = convert_random_forest(self.batch_shape[1], self.sklearn_model, self.flag_clf, self.dtype, self.target, self.dtype_converting, self.sparse_replacing)
            else:
        ...
        if(self.flag_clf == "forest_clf"):
            if(self.sparse_replacing in ["csr", "bsr", "csc"]):
                ...
                return classes, L, S_data, S_row, S_col, T, B, step
            elif(self.sparse_replacing == "dia"):
                return classes, L, S_data, S_offsets, T, B, step
            elif(self.sparse_replacing == "lil"):
                return classes, L, S_data, S_rows, S_non_zero_cols, T, B, step
            else:
            ...
            if(self.sparse_replacing in ["csr", "bsr", "csc"]):
                ...
                return L, S_data, S_row, S_col, T, B, step
            elif(self.sparse_replacing == "dia"):
                return L, S_data, S_offsets, T, B, step
            elif(self.sparse_replacing == "lil"):
                return L, S_data, S_rows, S_non_zero_cols, T, B, step
            else:
                return L, S, T, B, step
...
class SVMModel(BaseModel):
    ...
    def _parse_params(self):
        ...
        print(self.sklearn_model.n_support_)
