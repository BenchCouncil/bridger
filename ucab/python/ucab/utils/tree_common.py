...
from tvm import te
import numpy as np
from scipy.sparse import csr_matrix,bsr_matrix,dia_matrix,coo_matrix,csc_matrix,lil_matrix
...
def dense_to_csc(x, dtype):
    """
    Convert dense data to sparse data in csc format
    """
    x = csc_matrix(x)
    data = x.data.astype(dtype)
    indices = x.indices.astype("int32")
...
def dense_to_bsr(x, dtype):
    """
    Convert dense data to sparse data in bsr format
    """
    x = bsr_matrix(x)
    data = x.data.astype(dtype)
    indices = x.indices.astype("int32")
...
def dense_to_coo(x, dtype):
    """
    convert dense data to sparse data in coo format
    """
    x = coo_matrix(x)
    data = x.data.astype(dtype)
    row = x.row.astype("int32")
    col = x.col.astype("int32")
    data = tvm.nd.array(data)
    row = tvm.nd.array(row)
    col = tvm.nd.array(col)
    return data, row, col

def dense_to_dia(x, dtype):
    """
    convert dense data to sparse data in csr format
    """
    x = dia_matrix(x)
    data = x.data.astype(dtype)
    offsets = x.offsets.astype("int32")
    data = tvm.nd.array(data)
    offsets = tvm.nd.array(offsets)
    return data, offsets

def list_to_array(x, dtype):
    narray = np.zeros([x.size, len(max(x, key=lambda a: len(a)))], dtype=dtype)
    for i, j in enumerate(x):
        narray[i][:len(j)] = j
    return narray

def dense_to_lil(x, dtype):
    """
    convert dense data to sparse data in lil format
    """
    x = lil_matrix(x)
    data = list_to_array(x.data, dtype)
    rows = list_to_array(x.rows, "int32")
    non_zero_cols = np.array([len(x.data[i]) for i in range(0, x.data.shape[0])], dtype="int32")
    data = tvm.nd.array(data)
    rows = tvm.nd.array(rows)
    non_zero_cols = tvm.nd.array(non_zero_cols)
    return data, rows, non_zero_cols
...
def convert_decision_tree(X_shape, clf, clf_flag, dtype, target, sparse, type_convert):
    ...
    L = tvm.nd.array(L)
    if(sparse == "csr"):
        S_data, S_indices, S_indptr = dense_to_csr(S, "float32")
        print("*"*100)
        print(S_data, S_indices, S_indptr)
        return S_data, S_indices, S_indptr, T, B, L
    if(sparse == "bsr"):
        S_data, S_indices, S_indptr = dense_to_bsr(S, "float32")
        print("*"*100)
        print(S_data, S_indices, S_indptr)
        return S_data, S_indices, S_indptr, T, B, L
    if(sparse == "csc"):
        S_data, S_indices, S_indptr = dense_to_csc(S, "float32")
        print("*"*100)
        print(S_data, S_indices, S_indptr)
        return S_data, S_indices, S_indptr, T, B, L
    if(sparse == "coo"):
        S_data, S_row, S_col = dense_to_coo(S, "float32")
        print("*"*100)
        print(S_data, S_row, S_col)
        return S_data, S_row, S_col, T, B, L
    elif(sparse == "dia"):
        S_data, S_offsets = dense_to_dia(S, "float32")
        print("*"*100)
        print(S_data, S_offsets)
        return S_data, S_offsets, T, B, L
    elif(sparse == "lil"):
        S_data, S_rows, S_non_zero_cols = dense_to_lil(S, "float32")
        print("*"*100)
        print(S_data, S_rows, S_non_zero_cols)
        return S_data, S_rows, S_non_zero_cols, T, B, L
    else:
        S = tvm.nd.array(S)
        return S, T, B, L
...
def convert_random_forest(X_shape, clf, clf_flag, dtype, target, dtype_converting, sparse_replacing):
    ...
    LE = tvm.nd.array(LE)
    if(clf_flag == "forest_clf"):
        if(sparse_replacing == "csr"):
            SE_data, SE_indices, SE_indptr = dense_to_csr(SE, "float32")
            return SE_data, SE_indices, SE_indptr, TE, BE, step, LE, classes
        if(sparse_replacing == "bsr"):
            SE_data, SE_indices, SE_indptr = dense_to_bsr(SE, "float32")
            return SE_data, SE_indices, SE_indptr, TE, BE, step, LE, classes
        elif(sparse_replacing == "csc"):
            SE_data, SE_indices, SE_indptr = dense_to_csc(SE, "float32")
            return SE_data, SE_indices, SE_indptr, TE, BE, step, LE, classes
        elif(sparse_replacing == "coo"):
            SE_data, SE_row, SE_col = dense_to_coo(SE, "float32")
            return SE_data, SE_row, SE_col, TE, BE, step, LE, classes
        elif(sparse_replacing == "dia"):
            SE_data, SE_offsets = dense_to_dia(SE, "float32")
            return SE_data, SE_offsets, TE, BE, step, LE, classes
        elif(sparse_replacing == "lil"):
            SE_data, SE_rows, SE_non_zero_cols = dense_to_lil(SE, "float32")
            return SE_data, SE_rows, SE_non_zero_cols, TE, BE, step, LE, classes
        else:
            SE = tvm.nd.array(SE)
        ...
        if(sparse_replacing == "csr"):
            ...
            SE_data, SE_row, SE_col = dense_to_coo(SE, "float32")
            return SE_data, SE_row, SE_col, TE, BE, step, LE
        elif(sparse_replacing == "dia"):
            SE_data, SE_offsets = dense_to_dia(SE, "float32")
            return SE_data, SE_offsets, TE, BE, step, LE
        elif(sparse_replacing == "lil"):
            SE_data, SE_rows, SE_non_zero_cols = dense_to_lil(SE, "float32")
            return SE_data, SE_rows, SE_non_zero_cols, TE, BE, step, LE
        else:
            SE = tvm.nd.array(SE)
