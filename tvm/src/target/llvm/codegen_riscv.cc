...
/*!
 * \file codegen_RISCV.cc
 * \brief RISCV specific code generator
 */
...
// RISCV specific code generator, this is used as an example on
// how to override behavior llvm code generator for specific target
class CodeGenRISCV final : public CodeGenCPU {
 public:
  CodeGenRISCV() = default;
  virtual ~CodeGenRISCV() = default;
 ...
 private:
  PrimExpr RISCVPopcount(const CallNode* op);
};

llvm::Value* CodeGenRISCV::CreateIntrinsic(const CallNode* op) {
...
PrimExpr CodeGenRISCV::RISCVPopcount(const CallNode* call) {
  using namespace tir;
  const PrimExpr& e = call->args[2];
  llvm::Intrinsic::ID ctpop_id = llvm::Intrinsic::ctpop;
  llvm::Intrinsic::ID vpaddlu_id = llvm::Intrinsic::RISCV_neon_vpaddlu;
...
TVM_REGISTER_GLOBAL("tvm.codegen.llvm.target_riscv")
    .set_body([](const TVMArgs& targs, TVMRetValue* rv) {
      *rv = static_cast<void*>(new CodeGenRISCV());
    });
